...
def no_sales_criteria(df, n):
    if n == 0:
        ...
        return False
    elif df.iloc[-n:].values.sum() == 0.000001 * n:
                ...
                for tr in ['additive', 'multiplicative', None]:
                    for damp in [True, False]:
                        for ss in ['additive', 'multiplicative', None]:
                            ...
                            except (OverflowError, ValueError, IndexError, NotImplementedError):
                                continue
    return error_dict_X_S
...
def main_prediction(chain_list, category_list, channel, time_connection, status_name=0):
    ...
    data = pd.read_sql(f"SELECT * FROM [dbo].[SalesInWeek] "
                       f"WHERE cpg_id in {chain_str} and l3_id in {category_str}", connection)
    ...
    for chain in chain_name:
        ...
        df_final_full_reorder = df_final_full[['buyer_id',
                                               ...
                                               'predict_holt_wint',
                                               'predict_holt_wint_corr']]
        # df_final_full_reorder.to_csv('trer1.csv', mode='a')
        break
...
def main_prediction_week(df, coef_smoothing, filling_calendar='yes', start_period=0, end_period=datetime.now()):
    ...
    print(df.l3_id.value_counts())
    print(df.cpg_id.value_counts())
    l3 = 19.0
    chain = 418

    df_1 = df[(df.l3_id == l3) & (df.status_id == 2)]
    df_1 = df_1[['l3_id', 'l3_name', 'year', 'week', 'volume', 'status_id', 'cpg_id', 'cpg_name']]
    df_1 = df_1.groupby(by=['l3_id', 'l3_name', 'year', 'week', 'status_id', 'cpg_id', 'cpg_name'],
                        as_index=False).sum()
    ...
    start_week = df_1[df_1.year == start_year].week.min()
    final_year = df_1.year.max()
    final_week = 50

    # def sample_calendar_week
    res = []
    s = 1
    f = 52
    for y in range(start_year, final_year + 1):
    ...
    df_1.loc[(df_1['volume'] <= 0), 'volume'] = 0.000001
    ...
    for f_y in df_1.year.unique():
        ...
        for m in range(1, 53):
            ...
            if len(df_1[(df_1['year'] == f_y) & (df_1['week'] == m)]['volume']) == 0:
                ...
                seas_week[m] = seas_week[m] + df_1[(df_1['year'] == f_y) & (df_1['week'] == m)]['volume'].values[0] / \
    ...
    print(df_1)
    if no_sales_criteria(df_1.volume, 6):
        print("Stop")
        plt.plot(df_1.volume)
        plt.show()
    else:
        X = df_1['volume']
        X = X.reset_index(drop=True)
        ...
        best_params_founder(
            ...
            best_params_X)
        best_params_founder(
            Holt_Winters(train_X, test_X, 0.2, 0.2, 0.2, df_modeling_X.iloc[:-t], modeling=1, seasonal_period=52),
            best_params_X)
        best_params_founder(
...
if __name__ == '__main__':
    ...
    # 881
    # print(datetime.now())
    date_time_obj = datetime.strptime('2023-12-15 00:00:00.000', '%Y-%m-%d %H:%M:%S.%f')
    # date_time_obj = datetime.now()
    # print(date_time_obj)

    main_prediction_week(pd.read_csv('borchenko_test.csv', sep=';', decimal='.'), 1, 'yes')

    # main_prediction(
    #    chain_list=[2, 4, 336, 697, 737, 854, 1563],
    #    category_list=[],
    #    channel=38,
